...
namespace Bouyei.NetFactoryCore
{
    ...
    public class NetClientProvider : INetClientProvider
    {
        ...
        public bool Send(SegmentOffset dataSegment, bool waiting = true)
        {
            ...
            {
                return wsClientProvider.Send(dataSegment, waiting);
            }
            return false;
        }

        public bool Send(string content, bool waiting = true)
        {
            if (NetProviderType == NetProviderType.Tcp)
            {
                return tcpClientProvider.Send(new SegmentOffset(encoding.GetBytes(content)), waiting);
            }
            else if (NetProviderType == NetProviderType.Udp)
            {
                return udpClientProvider.Send(new SegmentOffset(encoding.GetBytes(content)), waiting);
            }
            else if (NetProviderType == NetProviderType.WebSocket)
            {
                wsClientProvider.Send(content);
            }
            return false;
        }
        public bool ConnectSync(int port, string ip)
        {
            ...
            {
              return  wsClientProvider.Connect("ws://" + ip + ":" + port);
            }
            return false;
        }
