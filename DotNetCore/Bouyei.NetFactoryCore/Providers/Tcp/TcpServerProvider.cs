...
namespace Bouyei.NetFactoryCore.Tcp
{
    using Base;
    using Providers.Tcp;
    ...
    public class TcpServerProvider : TcpSocket,IDisposable
    {
            ...
            :base(chunkBufferSize)
        {
            ...
            int cnt = maxConnections + offsetNumber;
            maxNumberAcceptedClients = new Semaphore(cnt, cnt);

            recvBufferManager = new SocketBufferManager(cnt, chunkBufferSize);
            acceptTokenManager = new SocketTokenManager<SocketAsyncEventArgs>(cnt);

            sendTokenManager = new SocketTokenManager<SocketAsyncEventArgs>(cnt);
            sendBufferManager = new SocketBufferManager(cnt, chunkBufferSize);
        }
        ...
        private void ProcessDisconnectCallback(SocketAsyncEventArgs e)
        {
            SocketToken sToken = e.UserToken as SocketToken;
            if (sToken == null) {
                return;// throw new Exception("关闭的对象为空");
            }
